...
if ( ! function_exists( 'pixwell_bookmark' ) ) :
	function pixwell_bookmark() {
		...
		?><span class="read-it-later bookmark-item" <?php if ( is_rtl() ) {
			echo 'dir="rtl"';
		}
	; ?> data-title="<?php echo pixwell_translate( 'read_later' ); ?>" data-bookmarkid="<?php echo get_the_ID(); ?>">
...
if ( ! function_exists( 'pixwell_bookmark_list_empty' ) ) {
	function pixwell_bookmark_list_empty() {
		...
		$response .= '<div class="bookmark-empty-icon"><i class="rbi rbi-warning" aria-hidden="true"></i></div>';
