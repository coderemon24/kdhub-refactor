...
if ( ! function_exists( 'rb_submit_newsletter' ) ) {
	function rb_submit_newsletter() {
		...
		if ( empty( $_POST['email'] ) || ! is_email( $_POST['email'] ) ) {
			wp_send_json( [ 'notice' => 'email-error' ] );
			die();
		}

		if ( isset ( $_POST['privacy'] ) && empty( $_POST['privacy'] ) ) {
			wp_send_json( [ 'notice' => 'privacy-error' ] );
			die();
		}
		...
		if ( in_array( $email, $subscribed_data ) ) {
			wp_send_json( [ 'notice' => 'email-exists' ] );
		}
	...
	function rb_render_newsletter( $settings ) {
		...
		<div class="<?php echo esc_attr( $class_name ) ?>">
			<div class="rb-newsletter-inner">
				<div class="newsletter-cover">
					<?php if ( ! empty( $settings['cover'] ) ) : ?>
						<img loading="lazy" decoding="async" src="<?php echo esc_url( $settings['cover']['url'] ); ?>" width="<?php echo esc_attr( $settings['cover']['width'] ) ?>" height="<?php echo esc_attr( $settings['cover']['height'] ) ?>" alt="<?php esc_attr( $settings['title'] ) ?>"/>
					<?php endif; ?>
				</div>
				<div class="newsletter-content">
					...
					if ( ! empty( $settings['description'] ) ) : ?>
						<div class="newsletter-desc"><?php pixwell_render_inline_html( $settings['description'] ); ?></div>
					<?php endif;
					...
					<form class="rb-newsletter-form" action="#" method="post">
						<div class="newsletter-input">
							<input class="newsletter-email" placeholder="<?php echo esc_attr( $settings['placeholder'] ); ?>" type="email" name="rb_email_subscribe">
							...
							<?php else : ?>
								<button type="submit" name="submit" class="newsletter-submit newsletter-submit-icon" aria-label="<?php esc_html_e( 'submit', 'pixwell-core' ); ?>">
						...
						<?php if ( ! empty( $settings['privacy'] ) ) : ?>
							<div class="newsletter-privacy">
								...
								<label for="rb-privacy-<?php echo esc_attr( $checkbox_id ); ?>"><?php pixwell_render_inline_html( $settings['privacy'] ); ?></label>
							</div>
						<?php endif; ?>
					</form>
				</div>
			...
			<div class="newsletter-response">
				<?php if ( ! empty( $settings['privacy'] ) ) : ?>
					<span class="response-notice privacy-error"><?php pixwell_render_inline_html( $settings['privacy_error'] ); ?></span>
				<?php endif; ?>
				<span class="response-notice email-error"><?php pixwell_render_inline_html( $settings['email_error'] ); ?></span>
				<span class="response-notice email-exists"><?php pixwell_render_inline_html( $settings['email_exists'] ); ?></span>
				<span class="response-notice success"><?php pixwell_render_inline_html( $settings['success'] ); ?></span>
			</div>
		</div>
		<?php
		return ob_get_clean();
