	...
	class pixwell_widget_youtube_subscribe extends WP_Widget {
		...
		function form( $instance ) {
			...
			<p>
				...
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'channel_name' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'channel_name' ) ); ?>" value="<?php if ( ! empty( $instance['channel_name'] ) ) {
					echo esc_attr( $instance['channel_name'] );
				} ?>"/>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'channel_ID' ) ); ?>"><?php esc_html_e( 'or Channel ID:', 'pixwell-core' ) ?></label>
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'channel_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'channel_id' ) ); ?>" value="<?php if ( ! empty( $instance['channel_id'] ) ) {
					echo esc_attr( $instance['channel_id'] );
				} ?>"/>
			</p>
			<?php
		}
