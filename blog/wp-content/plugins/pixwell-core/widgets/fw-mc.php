			...
			<div class="<?php echo esc_attr( $classes ); ?>" <?php echo $style; ?>>
				<div class="rbc-container inner rb-p20-gutter">
					<?php if ( ! empty( $instance['title'] ) ) : ?>
						<h5 class="mc-title h2"><?php pixwell_render_inline_html( $instance['title'] ); ?></h5>
					<?php endif;
					if ( ! empty( $instance['description'] ) ) : ?>
						<div class="mc-desc">
							<span><?php pixwell_render_inline_html( $instance['description'] ); ?></span></div>
					<?php endif;
		...
		function form( $instance ) {
			...
			<p>
				...
				<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'text_style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'text_style' ) ); ?>">
					<option value="0" <?php if ( ! empty( $instance['text_style'] ) && $instance['text_style'] == '0' ) {
						echo "selected=\"selected\"";
					} else {
						echo "";
					} ?>><?php esc_html_e( 'Light Text', 'pixwell-core' ); ?></option>
					<option value="1" <?php if ( ! empty( $instance['text_style'] ) && $instance['text_style'] == '1' ) {
						echo "selected=\"selected\"";
					} else {
						echo "";
					} ?>><?php esc_html_e( 'Dark Text', 'pixwell-core' ); ?></option>
				</select>
			</p>
			<p>
			...
			<p>
				...
				<span id="<?php echo esc_attr( $this->get_field_id( 'preview' ) ); ?>" class="w-img-preview">
						<?php if ( ! empty( $instance['image'] ) ) : ?>
							<img src="<?php echo wp_get_attachment_url( $instance['image'], 'full' ); ?>"><?php endif; ?>
				</span>
				...
				<input class="w-image-id" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" type="hidden" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" value="<?php echo esc_html( $instance['image'] ); ?>">
			</p>
			<?php
		}
	}
