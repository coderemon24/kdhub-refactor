...
if ( ! function_exists( 'pixwell_get_block_attributes' ) ) {
	function pixwell_get_block_attributes( $block_name, $post_id, $client_id ) {
...
if ( ! function_exists( 'pixwell_get_block_padding_css' ) ) {
	function pixwell_get_block_padding_css( $padding = [] ) {
...
if ( ! function_exists( 'pixwell_get_block_border_width_css' ) ) {
	function pixwell_get_block_border_width_css( $border = [] ) {
...
if ( ! function_exists( 'pixwell_block_review_schema_markup' ) ) {
	function pixwell_block_review_schema_markup( $attributes ) {
		...
		$item['average'] = pixwell_block_review_get_average( $attributes );
		if ( empty( $item['average'] ) && ! empty( $attributes['metaScore'] ) ) {
			$item['average'] = $attributes['metaScore'];
		} else {
			$item['average'] = 5;
		}
		...
		if ( ! empty( $attributes['buyButtons'] ) ) {
			...
			if ( ! empty( $attributes['salePrice'] ) ) {
				$item['price'] = pixwell_get_price_number( $attributes['salePrice'] );
			} elseif ( ! empty( $attributes['price'] ) ) {
				$item['price'] = pixwell_get_price_number( $attributes['price'] );
			}
		...
		pixwell_review_product_schema_markup( $item );
	}
}

if ( ! function_exists( 'pixwell_get_price_number' ) ) {
	function pixwell_get_price_number( $price ) {
...
if ( ! function_exists( 'pixwell_review_product_schema_markup' ) ) {
	function pixwell_review_product_schema_markup( $item ) {
