...
if ( ! class_exists( 'RUBY_COMPOSER' ) ) {
	class RUBY_COMPOSER {
		...
		public function __construct() {
			...
			add_action( 'init', [ $this, 'load_language' ], 20 );
			add_action( 'add_meta_boxes', [ $this, 'register_composer_meta' ], 0 );
			add_action( 'current_screen', [ $this, 'init' ] );
			add_filter( 'template_include', [ $this, 'frontend_redirect' ], 99 );
			add_filter( 'theme_page_templates', [ $this, 'frontend_template' ], 20 );
			add_filter( 'redirect_canonical', [ $this, 'pagination_redirect' ], 10 );
			add_action( 'save_post', [ $this, 'save_composer' ], 10 );
			add_action( 'save_post', [ $this, 'save_post_content' ], 20, 2 );
			add_action( 'wp_ajax_rbc_submit_template', [ $this, 'submit_template' ] );
			add_action( 'wp_ajax_rbc_delete_template', [ $this, 'delete_template' ] );
			add_action( 'wp_ajax_rbc_load_template', [ $this, 'load_template' ] );
			add_action( 'wp_enqueue_scripts', [ $this, 'register_frontend' ], 10 );
			add_filter( 'display_post_states', [ $this, 'show_composer_state' ] );
		}
		...
		public function init() {
			...
			if ( ( 'post.php' == $pagenow || 'post-new.php' == $pagenow ) && get_current_screen()->post_type == 'page' ) {
				...
				wp_enqueue_style( 'ruby-composer-style', RUBY_COMPOSER_URL . 'assets/main.css', [
					'wp-color-picker',
				], RUBY_COMPOSER_VERSION, 'all' );
				wp_register_script( 'ruby-composer-main', RUBY_COMPOSER_URL . 'assets/main.js', [
					'jquery',
					'wp-color-picker',
					'wp-util',
				], RUBY_COMPOSER_VERSION, true );
				add_action( 'admin_footer', [ $this, 'footer_loader' ], 9999 );
			}
		}
		...
		public function register_composer_meta() {
			...
			add_meta_box( 'rbc-global-meta', $section['title'], [
				$this,
		...
		public function register_frontend() {
			...
			if (  is_page_template( 'rbc-frontend.php' ) ) {
				if ( wp_style_is( 'pixwell-main' ) ) {
					...
					wp_enqueue_style( 'rbc-frontend', RUBY_COMPOSER_URL . 'assets/frontend.css', [], RUBY_COMPOSER_VERSION, 'all' );
		...
		public function footer_loader() {
			...
			wp_localize_script( 'ruby-composer-main', 'rbcParams', [ 'ajaxurl' => admin_url( 'admin-ajax.php' ) ] );
							...
							foreach ( $section['blocks'] as $block_id => $block ) {
								if ( ! empty( $block ) && is_array( $block ) ) {
									foreach ( $block as $field_id => $value ) {
										if ( ( ! empty( $field_id ) && 'html_' == substr( $field_id, 0, 5 ) ) || 'shortcode' == $field_id || 'raw_html' == $field_id || 'ad_script' == $field_id ) {
		...
		public function frontend_template( $page_templates ) {
			...
			$page_templates = array_merge( $page_templates, [
		...
		public function tab_loader() {
			...
			self::$tabs = [
				'filter'     => esc_html__( 'Query Settings', 'rbc' ),
				...
				'fullwidth' => [
					...
					'inputs'      => [
						[
							...
							'description' => esc_html__( 'Select a layout for this section', 'rbc' ),
							'options'     => [
								'full'      => esc_html__( 'Full Width (Inner Content: 1200px)', 'rbc' ),
								'wrapper'   => esc_html__( 'Wrapper (Boxed Section and Inner Content: 1200px)', 'rbc' ),
								'stretched' => esc_html__( 'Full Width Stretched (Full-Wide Content: 100%)', 'rbc' ),
							],
							'default'     => 'full',
						],
						[
							...
							'description' => esc_html__( 'Select margin top and bottom values (in pixels) for this section. Default is 50px.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'bottom' => 50,
							],
						...
						[
							...
							'description' => esc_html__( 'Select padding value (in pixels) for this section. Default is 0.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'right'  => 0,
								'bottom' => 0,
						...
						[
							...
							'description' => esc_html__( 'Select margin top and bottom values (in pixels) for this section on mobile devices. Default is 35px.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'bottom' => 35,
							],
						...
						[
							...
							'description' => esc_html__( 'Select padding value (in pixels) for this section on mobile devices. Default is 0.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'right'  => 0,
								'bottom' => 0,
						...
						[
							...
							'description' => esc_html__( 'Select the background color for this section.', 'rbc' ),
						],
						[
							'name'        => 'bg_image',
							'type'        => 'text',
							'title'       => esc_html__( 'Background Image', 'rbc' ),
							'description' => esc_html__( 'Input a background image (attachment link) for this section (.png or .jpg at the end).', 'rbc' ),
							'default'     => '',
						],
						[
							'name'        => 'bg_display',
							...
							'description' => esc_html__( 'Select the background display for this section (if you use a background image).', 'rbc' ),
							'options'     => [
						...
						[
							...
							'description' => esc_html__( 'Select the background position for this section (if you use a background image).', 'rbc' ),
							'options'     => [
				...
				'content'   => [
					...
					'inputs'      => [
						[
							...
							'type'        => 'dimension',
							'title'       => esc_html__( 'Margin', 'rbc' ),
							'description' => esc_html__( 'Select the top and bottom margin values (in pixels) for this section. Default is 50px.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'bottom' => 50,
							],
						...
						[
							...
							'type'        => 'dimension',
							'title'       => esc_html__( 'Padding', 'rbc' ),
							'description' => esc_html__( 'Select the padding value (in pixels) for this section. Default is 0.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'right'  => 0,
								'bottom' => 0,
						...
						[
							...
							'type'        => 'dimension',
							'title'       => esc_html__( 'Mobile Margin', 'rbc' ),
							'description' => esc_html__( 'Select the top and bottom margin values (in pixels) for this section on mobile devices. Default is 35px.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'bottom' => 35,
							],
						...
						[
							...
							'type'        => 'dimension',
							'title'       => esc_html__( 'Mobile Padding', 'rbc' ),
							'description' => esc_html__( 'Select the padding value (in pixels) for this section on mobile devices. Default is 0.', 'rbc' ),
							'default'     => [
								'top'    => 0,
								'right'  => 0,
								'bottom' => 0,
		...
		public function process_data( $input_data ) {
			...
			foreach ( $sections as $section_id ) {
				...
				$composer_data[ $section_id ]           = [];
				$composer_data[ $section_id ]['uuid']   = $section_id;
				$composer_data[ $section_id ]['blocks'] = [];
				...
				if ( ! empty( $input_data['rbc_block_order'][ $section_id ] ) && is_array( $input_data['rbc_block_order'][ $section_id ] ) ) {
					...
					foreach ( $blocks as $block_id ) {
						$composer_data[ $section_id ]['blocks'][ $block_id ]         = [];
		...
		public function save_post_content( $page_id, $post ) {
			...
			remove_action( 'save_post', [ $this, 'save_post_content' ], 20 );
