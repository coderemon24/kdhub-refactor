		...
		switch ( $directory ) {
			...
			case '17' :
				return [
					...
					[
						'name' => esc_html__( 'Cooked – Recipe Management', 'pixwell-importer' ),
						'slug' => 'cooked',
						'info' => esc_html__( 'Recommended', 'pixwell-importer' ),
					],
					[
			...
			case '46' :
				return [
					...
					[
						'name' => esc_html__( 'Simple Membership', 'pixwell-importer' ),
			...
			case '47' :
				return [
					[
						'name'  => esc_html__( 'WP Recipe Maker', 'pixwell-importer' ),
...
if ( ! function_exists( 'pixwell_importer_demo_name' ) ) {
	function pixwell_importer_demo_name( $directory ) {
		...
		switch ( $directory ) {
			...
			case '43' :
				return esc_html__( 'Baby Care Concept', 'pixwell-importer' );
			case '44' :
				return esc_html__( 'Breaking News Concept', 'pixwell-importer' );
			case '45' :
				return esc_html__( 'Wellness Concept', 'pixwell-importer' );
			case '46' :
				return esc_html__( 'Today Concept', 'pixwell-importer' );
			case '47' :
				return esc_html__( 'Recipe Maker', 'pixwell-importer' );
			default:
				return $directory;
		}
	}
...
if ( ! function_exists( 'pixwell_importer_duplicate_menus' ) ) {
	function pixwell_importer_duplicate_menus() {
		...
		$deleted_menus = [ 'main', 'top', 'footer', 'footer-col-1', 'footer-col-2', 'footer-col-3' ];
...
if ( ! function_exists( 'pixwell_importer_init_widgets' ) ) {
	function pixwell_importer_init_widgets() {
		...
		//empty all sidebars
		$sidebars_widgets['pixwell_sidebar_default']       = [];
		$sidebars_widgets['pixwell_sidebar_offcanvas']     = [];
		$sidebars_widgets['pixwell_sidebar_topsite']       = [];
		$sidebars_widgets['pixwell_sidebar_single_top']    = [];
		$sidebars_widgets['pixwell_sidebar_single_bottom'] = [];
		$sidebars_widgets['pixwell_sidebar_fw_footer']     = [];
		$sidebars_widgets['pixwell_sidebar_footer_1']      = [];
		$sidebars_widgets['pixwell_sidebar_footer_2']      = [];
		$sidebars_widgets['pixwell_sidebar_footer_3']      = [];

		/** add sidebars */
		$theme_options                          = get_option( 'pixwell_theme_options' );
		$theme_options['pixwell_multi_sidebar'] = [ 'sb1', 'sb2', 'infinite', 'single' ];
	...
	function pixwell_importer_header() {
		...
		<div class="importer-header">
			...
			<div class="importer-desc">
				<p>Importing the theme demo allows you to quickly edit everything instead of creating content from
					scratch. Please <strong>DO NOT navigate away</strong> from this page while the importer is
					processing. This may take up to 5 ~ 7 minutes, depending on the server speed.</p>
				<p>We do not have the right to include some images of demos in the content due to copyright issues, so images
					will look different from the demo. The structures of demos will still be left intact so you can use your
			...
			<div class="importer-tips">
				...
				<p>- You can choose to import Only Pages, Widgets, and Theme Options if your site already has data.</p>
				<p>-<strong>Don't need</strong> to install or activate Recommended & Optional plugins if you don't want
					to use them.
				</p>
				<p>- Install and activate the Woocommerce plugin before importing if you would like to set up a shop (Fashion
					Concept).</p>
				<p>- You can <strong>disable or delete</strong> the IMPORTER plugin after completion.</p>
				<p>- Online Documentation:
					<a href="http://help.themeruby.com/pixwell" target="_blank">HERE</a>
				</p>
			</div>
		</div>
		<?php
